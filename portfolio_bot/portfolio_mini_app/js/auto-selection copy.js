...
const goalPaths = {
    grow: ['step-grow-amount', 'step-grow-term', 'step-risk'],
    dream: ['step-dream-target', 'step-dream-initial', 'step-dream-term', 'step-risk'],
    passive: ['step-passive-term', 'step-passive-income', 'step-passive-initial', 'step-risk']
};
...
function setupEventListeners() {
        ...
        button.addEventListener('click', () => {
            ...
            switch (currentStepId) {
                ...
                case 'step-passive-initial':
                    stepData.value = state.investmentData.amount;
                    break;
                case 'step-grow-term':
                case 'step-dream-term':
                case 'step-passive-term':
                    stepData.value = state.investmentData.term_months;
                    break;
                case 'step-dream-target':
                    stepData.value = state.investmentData.dreamAmount;
                    break;
                case 'step-passive-income':
                    stepData.value = state.investmentData.passiveIncome;
                    break;
            }
    ...
    document.getElementById('confirm-risk-btn').addEventListener('click', () => {
        collectDataFromStep('step-risk');
        trackEvent('confirm_risk_and_build', state.investmentData);
    ...
    document.querySelectorAll('input[type="range"], input[type="number"]').forEach(element => {
        element.addEventListener('input', () => {
             if (element.type === 'range' && element.id.startsWith('term-')) {
                ...
                if (valueSpan) {
                    valueSpan.textContent = formatTermForSlider(element.value);
                }
             }
    ...
    document.getElementById('income-passive').addEventListener('input', () => {
    ...
    setupSlider('term-grow', 'term-value-grow');
    setupSlider('term-dream', 'term-value-dream');
    setupSlider('term-passive', 'term-value-passive');
...
function initializeChart() {
    ...
    chartInstance = new Chart(ctx, { type: 'line', data: {}, options: getChartOptions() });
}
...
// --- ИЗМЕНЕНИЕ: Функции предпросмотра теперь работают с месяцами ---
...
async function makeApiCallAndUpdateChart() {
    if (state.history[state.history.length-1] !== 'step-risk') {
        backendDataCache = null;
        return;
    }
...
function setupSlider(sliderId, valueSpanId) {
    ...
    const valueSpan = document.getElementById(valueSpanId);
    if (slider && valueSpan) {
        valueSpan.textContent = formatTermForSlider(slider.value);
    }
}
